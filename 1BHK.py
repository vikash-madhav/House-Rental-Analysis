...
for house in houses:
    ...
    for ul in house.findAll('ul', class_='i_l clearfix'):
        ...
        amenities.append(ul)
    for ul in amenities:
        IBHKHouse_details["Amenities"].append(ul.text)
    ...
    IBHKHouse_details["Details"].append(details)
    IBHKHouse_details["Agent"].append(agent)
    IBHKHouse_details["Rent"].append(rent)
    IBHKHouse_details["Location"].append(location)
    IBHKHouse_details["Seller"].append(seller)
    IBHKHouse_details["Carpet_area"].append(carpet_area)

    i += 1

    # Converting the dictionary into a pandas DataFrame and then to CSV file
df = pd.DataFrame.from_dict(IBHKHouse_details, orient="index")
df = df.transpose()
df.to_csv("IBHKHouse_details.csv")
df = pd.read_csv("IBHKHouse_details.csv", index_col=0)
df.tail(5)
